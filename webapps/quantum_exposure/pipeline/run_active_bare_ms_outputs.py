#!/usr/bin/env python3
"""Update active_bare_ms_outputs incrementally."""
...
TARGET_TABLE = "active_bare_ms_outputs"
SCHEMA = "public"
...
def seed_changed_addresses_from_outputs(cur, previous_freeze: int, freeze_height: int):
    cur.execute(
        ...
        WHERE o.scripttype LIKE 'Multisig %%'
          AND o.blockheight > %s
          AND o.blockheight <= %s
...
def seed_changed_addresses_from_latest_stxo_new_unspent(cur, latest_part: StxoPartition, previous_freeze: int, freeze_height: int):
    cur.execute(
        ...
        WHERE s.scripttype LIKE 'Multisig %%'
          AND s.blockheight > %s
          AND s.blockheight <= %s
        ...
        ON CONFLICT DO NOTHING;
        """,
        (previous_freeze, freeze_height, freeze_height),
    )
...
def upsert_historical_rows_from_partition(cur, part: StxoPartition, freeze_height: int) -> int:
    cur.execute(
        ...
        SET address = EXCLUDED.address,
            ...
            spendingblock = EXCLUDED.spendingblock;
        """,
        (freeze_height, freeze_height, freeze_height),
    )
    return cur.rowcount
    ...
    try:
        with conn.cursor() as cur:
            ...
            if previous_freeze is None:
                raise RuntimeError(
                    "No analysis_freeze row found for 'active_bare_ms_outputs'. Run the initial build first or seed the checkpoint manually."
                )
